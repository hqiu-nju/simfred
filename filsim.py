...
##### these are model specific parameters
    ...
    fluence_fac = 18./2.  ### jansky  to units of sigma? which is 18 digits per 2 jansky
    ...
    for i in xrange(10):
        ...
        for j in range(336):
            ...
            snr_sig+=sum(abs((datasetsum[j][pulse]*npos/128)**2-np.mean(datasetsum[j]/128)**2))
            snr_bkg+=np.sqrt(len(npos)*np.var((datasetsum[j]/128)**2))
...
#def snr_cal(noise_base,pulse_map,mixed,nchan):
